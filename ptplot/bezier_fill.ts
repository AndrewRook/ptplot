...
export type BezierFillData = GlyphData & p.UniformsOf<BezierFill.Mixins> & {
  _x0: FloatArray
  _y0: FloatArray
  _cx0: FloatArray
  _cy0: FloatArray
...
export class BezierFillView extends GlyphView {
  model: BezierFill
  ...
  protected _index_data(index: SpatialIndex): void {
    const {data_size, _x0, _y0, _cx0, _cy0, _cx1, _cy1} = this
    ...
    for (let i = 0; i < data_size; i++) {
      ...
      if (isNaN(x0_i + y0_i + x0_i + y0_i + cx0_i + cy0_i + cx1_i + cy1_i))
        index.add_empty()
      else {
        const [x0, y0, x1, y1] = _cbb(x0_i, y0_i, x0_i, y0_i, cx0_i, cy0_i, cx1_i, cy1_i)
  ...
  protected _render(ctx: Context2d, indices: number[], data?: BezierFillData): void {
    const {sx0, sy0, scx0, scy0, scx1, scy1} = data ?? this
    for (const i of indices) {
      ...
      if (isNaN(sx0_i + sy0_i + scx0_i + scy0_i + scx1_i + scy1_i))
        continue

      ctx.beginPath()
      ctx.moveTo(sx0_i, sy0_i)
      ctx.bezierCurveTo(scx0_i, scy0_i, scx1_i, scy1_i, sx0_i, sy0_i)
...
export class BezierFill extends Glyph {
